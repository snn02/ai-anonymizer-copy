...
2. сделайте копию в `config.yaml`;
3. заполните `config.yaml` значениями под ваше окружение.
...
Вариант A: файл в дефолтном месте и с дефолтным именем `./config.yaml` (можно без `--config`).
...
2. Если `--config` не указан, runtime ищет `./config.yaml` в текущей рабочей папке.
