...
- Тип: изменение
- Описание: уточнен `user-guide` по config-first запуску: зафиксировано дефолтное расположение `./config.example.yaml`, добавлены варианты запуска без `--config` (при дефолтном имени/пути) и с `--config` (для отдельного файла), для `run` явно указан обязательный аргумент `<id|path>`.
- Ссылка: `../../user-guide.md`

### 2026-04-26

- Тип: изменение
- Описание: уточнен раздел секретов в `user-guide`: явно разделены идентификаторы (`api.partner_id`, `audit.hmac_key_id`) и секретные значения (`API_TOKEN`, `HMAC_SECRET`), команды для Windows/Linux/macOS синхронизированы с runtime-поиском секрета по паре `service + account(user)`.
- Ссылка: `../../user-guide.md`

### 2026-04-26

- Тип: изменение
- Описание: в `user-guide` возвращена явная команда генерации `HMAC_SECRET` через PowerShell (Base64, 32 байта) в разделе настройки секретов.
- Ссылка: `../../user-guide.md`

### 2026-04-26

- Тип: изменение
- Описание: в `user-guide` добавлено явное предупреждение по Windows-путям в YAML: запрещенные `\` в двойных кавычках и безопасные варианты записи путей для исключения ошибки `unknown escape character`.
- Ссылка: `../../user-guide.md`

### 2026-04-27

- Тип: изменение
- Описание: открыт подэтап `v1-1-ws` для MVP в изолированных AI IDE: в общей документации зафиксировано временное insecure-исключение (auth-токен из env/config при явном профиле), non-production ограничение и компенсирующие security-меры.
- Ссылка: `plan.md`

### 2026-04-27

- Тип: изменение
- Описание: создан OpenSpec feature-пакет `v1-1-ws-insecure-mvp-auth` (`proposal/tasks/spec`) и синхронизирован индекс `docs/versions/v1/openspec.md`; зафиксирована трассировка требований и security DoD перед стартом разработки.
- Ссылка: `openspec.md`

### 2026-04-27

- Тип: изменение
- Описание: требования `v1-1-ws` расширены с учетом audit-секрета: в документации и OpenSpec зафиксировано, что для временного insecure-профиля через env/config задаются оба значения — `api.auth_token` и `audit.hmac_secret`.
- Ссылка: `../../../openspec/changes/v1-1-ws-insecure-mvp-auth/tasks.md`

### 2026-04-27

- Тип: изменение
- Описание: OpenSpec-пакет `v1-i4-config-first-loading` закрыт и перенесен из `openspec/changes` в `openspec/archive`; индекс `docs/versions/v1/openspec.md` синхронизирован, `v1-i4` снят из активных.
- Ссылка: `openspec.md`

### 2026-04-29

- Тип: изменение
- Описание: синхронизирован `user-guide` с актуальным runtime по профилям секретов: явно разделены правила для стандартного `v1` (только OS secret store) и временного `v1-1-ws` (только при явном включении `runtime-profile=v1-1-ws` + `insecure_no_secrets=true` с `api.auth_token` и `audit.hmac_secret`); добавлены точные команды запуска и профильный чек-лист.
- Ссылка: `../../user-guide.md`

### 2026-04-29

- Тип: изменение
- Описание: синхронизированы тестовые сценарии с документацией и runtime: в `T6` уточнен контекст OS secret store для стандартного `v1`, в `T12` добавлена явная проверка включения `v1-1-ws` через параметры профиля/insecure-флаг.
- Ссылка: `../../test-scenarios.md`

### 2026-04-29

- Тип: изменение
- Описание: запланирована и реализована мини-итерация `v1-i5-http-debug-log`: добавлен опциональный диагностический HTTP-лог для `doctor/run`, включаемый только через `ANON_HTTP_DEBUG=true`, с записью в `<workspace>/.anonym/http-debug.log` и маскированием `Authorization`.
- Ссылка: `../../../openspec/changes/archive/2026-04-29-v1-i5-http-debug-log/tasks.md`

### 2026-04-29

- Тип: реакция
- Описание: синхронизированы пользовательская и тестовая документация по HTTP debug-режиму (`user-guide`, `T13` в `test-scenarios`), подтверждено тестами создание лога при включении и отсутствие лога при выключении флага.
- Ссылка: `../../test-scenarios.md`
